...
impl FromStr for Map {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        for line in lines {
            ...
            if nchar != ncol { return Err(NotRectangular) }
        }
...
impl Display for Map {
    fn fmt(&self, f: &mut Formatter) -> FmtResult {
        ...
        for row in self.data.chunks(self.ncol) {
            for cell in row {
                if cell.alive { map_str.push_str(" X"); }
            ...
            for j in 0..self.ncol {
                ...
                for m in i.saturating_sub(1)..min(i + 2, self.nrow) {
                    for n in j.saturating_sub(1)..min(j + 2, self.ncol) {
                        if self[(m, n)].alive && (m != i || n !=  j) { live_neighbours += 1; }
                    }
                }
                let cell = &mut self[(i, j)];
                if !cell.alive {
                    if live_neighbours == 3  { cell.lives = true; }
                }
                else if live_neighbours < 2 || 3 < live_neighbours { cell.lives = false; }
            }
        }
...
impl Map {
    fn from_parts_unchecked(cells: Vec<Cell>, (nrow, ncol): (usize, usize)) -> Self {
        ...
        Map { data: cells, nrow: nrow, ncol: ncol }
    }
}
